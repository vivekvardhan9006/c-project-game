...
games_again:
    ...
    if (game == 1) {
        ...
        do {
            drawBoard();
            player = (player % 2) ? 1 : 2;
            printf("Player %d won the toss and will play first.\n", tossResult);
            error_tic:
    ...
    int play_games;
    printf("Do you want to play other games\n");
...
int toss() {
    system("cls");
    // Simulate a toss. Return 1 for player 1 and 2 for player 2.
    printf("Tossing the coin...\n");
    Sleep(2000); // Sleep for 2 seconds to simulate the toss
    srand(time(NULL));
    return (rand() % 2) + 1;
}
